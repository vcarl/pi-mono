...
const testDir = path_1.default.join(packageRoot, "test-e2e-tmp");
console.log("🧪 E2E Test: Agent Self-Improvement\n");
// Step 1: Clean and build
console.log("📦 Step 1: Building package...");
try {
    (0, child_process_1.execSync)("npm run clean && npm run build", { cwd: packageRoot, stdio: "inherit" });
...
let tarballPath;
try {
    const output = (0, child_process_1.execSync)("npm pack", { cwd: packageRoot, encoding: "utf-8" });
    const tarballName = output.trim().split("\n").pop();
    tarballPath = path_1.default.join(packageRoot, tarballName);
...
try {
    ...
    fs_1.default.writeFileSync(path_1.default.join(testDir, "package.json"), JSON.stringify(testPackageJson, null, 2));
...
try {
    (0, child_process_1.execSync)(`npm install ${tarballPath} @mariozechner/pi-ai`, {
        cwd: testDir,
        stdio: "inherit",
    });
...
`;
try {
    fs_1.default.writeFileSync(path_1.default.join(testDir, "test-agent.js"), agentScript);
...
try {
    (0, child_process_1.execSync)("node test-agent.js", {
        cwd: testDir,
        stdio: "inherit",
